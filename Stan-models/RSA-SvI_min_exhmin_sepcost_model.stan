functions {
  ...
  vector posterior_parses_late(
    ...
    data vector U1_not_very_late_exhmin,
    data vector log_prior,
    data array[,] int ii
  ) {
    ...
    vector[K] S1_not_very_late_exhmin;
    vector[K] S1_const;
    ...
    vector[K] L1_exhmin_not_very_late;
    // normalizing constants for L1 (log scale):
    real const_min_late;
    real const_pos_late;
    real const_lit_not_very_late;
    real const_exhpos_not_very_late;
    real const_exhmin_not_very_late;
    // posterior parameters on logit scale
    real post_logit_min;
    real post_logit_exhpos;
    real post_logit_exhmin;
    // real post_exh;
    for (k in 1:K) {
    ...
    S1_not_very_late_exhmin = alpha_pos .* (U1_not_very_late_exhmin-costs[2]-costs[3]);
    // Compute the normalizing constant and normalize:
    for (k in 1:K) {
      S1_const[k] = log_sum_exp({
        S1_null_late[k],
    ...
    S1_not_very_late_exhmin = S1_not_very_late_exhmin-S1_const;
    // Compute unnormed logL1:
    ...
    L1_exhmin_not_very_late = S1_not_very_late_exhmin + log_prior;
    ...
    const_exhmin_not_very_late = log_sum_exp({log_sum_exp(L1_exhmin_not_very_late[ii[1]]),log_sum_exp(L1_exhmin_not_very_late[ii[2]]+log(3)), log_sum_exp(L1_exhmin_not_very_late[ii[3]]+log(3)),log_sum_exp(L1_exhmin_not_very_late[ii[4]])});
    // Compute posteriors on MIN and EXH_POS and EXH_MIN (alt: EXH), on logit scale:
    post_logit_min = const_min_late-const_pos_late;
    post_logit_exhpos = -log_sum_exp(const_exhmin_not_very_late-const_exhpos_not_very_late,const_lit_not_very_late-const_exhpos_not_very_late);
    post_logit_exhmin = -log_sum_exp(const_exhpos_not_very_late-const_exhmin_not_very_late,const_lit_not_very_late-const_exhmin_not_very_late);
    return [post_logit_min,post_logit_exhpos,post_logit_exhmin]';
  }
}
...
data {
  ...
  vector[K] U1_not_very_late_exhmin;
...
parameters {
  ...
  real m_cost_sen_tall;      // mean cost for tall
  real m_cost_sen_late;      // mean cost for late
...
transformed parameters {
  ...
  array[S] real post_logit_exhmin; // posterior probability of the exp_min parse for 'not very adj' on logit scale
  ...
  for (s in 1:S) {
    ...
    cost_sen[s] = (subj_adj[s] ? m_cost_sen_late : m_cost_sen_tall) + v[2,s];
    ...
    if (subj_adj[s]) { // late case
      vector[3] loc_post_param; // local variable to store posterior parameters
      loc_post_param = posterior_parses_late(
        ...
        U1_not_very_late_exhpos,U1_not_very_late_exhmin,log_prior_late,ii
      );
      post_logit_min[s] = loc_post_param[1];
      post_logit_exhpos[s] = loc_post_param[2];
      post_logit_exhmin[s] = loc_post_param[3];
    } else { // tall case
  ...
  post_lit = exp(log_inv_logit_diff(-to_vector(post_logit_exhpos),to_vector(post_logit_exhmin)));
  for (i in 1:N) {
...
model {
  ...
  m_cost_sen_tall ~ normal(0,2); // quite uninformative prior
  m_cost_sen_late ~ normal(0,2); // quite uninformative prior
